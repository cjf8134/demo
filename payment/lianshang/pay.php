  ...
  $log=new Log();
    if($_POST){
        ...
        if($sign==$post['sign']){
            ...
            $field['ordernumber'] = $post['orderNo'];//商品订单号
            $field['hrefbackurl']=$post['pickupUrl'];//支付成功后跳转的页面
            $field['callbackurl']=$notify_url;//异步支付结果通知页面
            $field['paymoney']=number_format($post['orderAmount'],2,".","");//金额（元）
            ...
            <form action="paying.php" method="post" class="form form-horizontal responsive" id="demoform">
                ...
                <div class="row cl">
                    ...
                    <div class="formControls col-xs-8">
                        <?php echo $field['ordernumber'];?>
                    </div>
                </div>
                <div class="row cl">
                    <label class="form-label col-xs-3">金额：</label>
                    <div class="formControls col-xs-8">
                        <?php echo $field['paymoney'];?>
                    </div>
                </div>
