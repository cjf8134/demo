...
if($post){
	$log->W("支付通知验证-参数:\n".var_export($post,true));
	$signature = $post['fxsign'];
	$strData = md5($post['fxstatus'] . $post['fxid'] . $post['fxddh'] . $post['fxfee'] . APP_KEY); //验证签名
	$log->W("支付通知验证-生成的签名:\n".$strData);
	//签名正确
	...
	$log->W("支付通知验证-参数:\n".var_export($post,true));
	$log->W("支付通知验证-结果:\n".($verify ? "ok" : "fail"));
	if ($verify && $post['fxstatus'] == 1) { // 签名验证通过
...
//		$order_no= $cache->get($post['fxddh']); //获取数据
		...
		$para["orderNo"]=$post['fxattch'];                              //订单号
		$para["orderAmount"]= strval($post['fxfee']);		            //订单金额(元)
		$para["orderCurrency"]='CNY';							        //货币类型
		$para["transactionId"]=$post['fxorder'];	                    //第三方平台订单号
