...
//            );
            ...
            if ($util->verifySign($respAr, $merconfig["Url_Param_Connect_Flag"], $removeKeys
                ...
                if (strcmp($respAr['RspCod'],"00000") == 0
                    ...
                    //收银台URL 拼接
                    $payUrl =  buildPayUrl($merconfig,$removeKeys,$base64Keys,$respAr['Token'],null,null,$post['customerId']);
                    if (empty($payUrl)){
  ...
  function buildPayUrl($merconfig,$removeKeys,$base64Keys,$token,$productId,$directBankId,$customerId){
      $util = new Util();
      if (empty($token)){
