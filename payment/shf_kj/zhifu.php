    ...
    if($_POST){
        ...
        $post=$_POST;
			   ...
			   $mer_private_key = openssl_pkey_get_private($merchant_private_key);
				...
				http://www.babaodao.com/api/pay/quickPay
				list($return_code, $json_return_content) = http_post_data($server_url."/api/pay/quickPay?context=".$cryptos, $json_context);
				$log->W('表单提交返回');
				$log->W(var_export($json_return_content,true));
				$json_return_content = json_decode($json_return_content,true);
				if($json_return_content)
				{
