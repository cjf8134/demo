  ...
  $log=new Log();
    if($_POST){
        ...
        if($sign==$post['sign']){
            ...
            $cache->set($orderNo, $post['orderNo']);  //保存数据
            $field['out_trade_no'] = $orderNo;//商品订单号
            $field['return_url']=$post['pickupUrl'];//支付成功后跳转的页面
            $field['notify_url']=$notify_url;//异步支付结果通知页面
            $field['total_fee']=number_format($post['orderAmount'],2,".","")*100;//金额（元）
            ...
            <form action="paying.php" method="post" class="form form-horizontal responsive" id="demoform">
                ...
                <div class="row cl">
                    ...
                    <div class="formControls col-xs-8">
                        <?php echo $field['out_trade_no'];?>
                    </div>
                </div>
                <div class="row cl">
                    <label class="form-label col-xs-3">金额：</label>
                    <div class="formControls col-xs-8">
                        <?php echo $field['total_fee']/100;?>
                    </div>
                </div>
                ...
                <div class="row cl">
                    ...
                    <div class="formControls col-xs-8">
							<span class="select-box">
									<select class="select" size="1" name="bankno">
