...
if($input){
	...
	$obj = xmltoarray($input);  // 将返回的xml 数据 转换为数组

	$log->W("支付通知验证-参数:\n".var_export($obj,true));
	$signature = $obj['sign'];
	$obj['sign'] = '';
	$log->W("支付通知验证-参数:\n".$obj);
	$strData = md5sign($obj,APP_KEY);
	//签名正确
	...
	if ($verify && $obj['return_code'] == "SUCCESS" && $obj['result_code'] == "SUCCESS") { // 签名验证通过
		...
		$para["orderNo"]=$config['orderNo'];		                    //订单号
		$para["orderAmount"]= strval($obj['total_fee']);		        //订单金额(元)
		$para["orderCurrency"]='CNY';							        //货币类型
		$para["transactionId"]=$obj['transaction_id'];			        //第3方支付流水
