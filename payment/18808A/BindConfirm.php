    ...
    $log->W(var_export($post, true));
     if(empty($post['protocolId'])){
            ...
            if($res['resultCode'] == "000000") {
                ...
                //            );
                $result = $cache->get("customerId_".$res['userId']); //获取数据
                if (!$result || empty($result['userId']) || empty($result['protocolId'])) {
                    ...
                    $cache->set("customerId_".$res['userId'], $data);  //保存数据
                }
...
$log->W('确定下单下单传入进来的参数');
$log->W(var_export($post_data, true));
        ?>
            <form name="pay" id="register_form" action='pay1.php' method='post'>
                <?php foreach($post_data as $name=>$value) {
