...
//                            $user = $cache->get("customerId_".$post['customerId']); //获取用户信息
//                            if(!$user || empty($user['userId']) || empty($user['protocolId'])){
//                                $user_data = array(
//                                    'userId'     => 1,
//                                    'protocolId' => 'jpd172KcQ',
//                                );
//                                $cache->set("customerId_".$post['customerId'], $user_data);  //保存数据
//                            }
                        $user = $cache->get("customerId_".$post['customerId']); //获取用户信息
                    if($user &&  !empty($user['userId']) && !empty($user['protocolId']) && $user['userId'] == $post['customerId']){
                            ...
                            $field['orderAmount']  =strval($post['orderAmount'] * 100);//金额（元）
