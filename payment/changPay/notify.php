...
if($post){
	$attach = $post["attach"];	//订单描述
	$signature = $post['sign'];
	$log->W("支付通知验证-返回的签名:\n".$signature);
	$strData = sprintf("partner=%s&ordernumber=%s&orderstatus=%s&paymoney=%s%s", M_ID, $post["ordernumber"], $post["orderstatus"], $post["paymoney"], APP_KEY); //连接字符串加密处理
