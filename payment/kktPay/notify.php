...
if($input){
	...
	$ret = json_decode($input['ret'],true);
	$post = json_decode($input['msg'],true);
	$signtrue = $input['sign'];
	$log->W("支付通知验证-参数res:\n".var_export($res,true));
	...
	if ($verify) { // 签名验证通过
		...
		$para["orderNo"]=$post['no'];
		//订单号
		$para["orderAmount"]= strval($post['money'] / 100);		            //订单金额(元)
		$para["orderCurrency"]='CNY';							        //货币类型
		$para["transactionId"]=$post['payNo'];			               //第3方支付流水
