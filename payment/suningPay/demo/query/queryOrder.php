    ...
    <form name=payForm action=queryOrderApi.php method=post target="_blank">
        <table class="table-data" width="100%" border="0" cellspacing="0" cellpadding="0">
            ...
            <tr>
                <th>&nbsp;</th>
                <td>
                <button class="btn-blue" type="button" style="text-align:center;" onclick="query();">查 询</button>
                </td>
            </tr>
        </table>
        </form>
...
function query()
{
	...
	$(window.parent.document).find("#resultframe").attr("src",'queryOrderApi.php?outOrderNo=' + outOrderNo + "&orderTime=" + orderTime);
}
</script>
</html>