...
class YbPay {
	...
	public function prepareSign($data) {
		//查单
		if($data['apiName'] == 'TRAN_QUERY') {
			$result = sprintf(
				"apiName=%s&apiVersion=%s&platformID=%s&merchNo=%s&orderNo=%s&tradeDate=%s",
				$data['apiName'], $data['apiVersion'], $data['platformID'], $data['merchNo'], $data['orderNo'], $data['tradeDate']
			);
			return $result;
			
			
		//下单
		} else if ($data['apiName'] == 'WWW_PAY' || $data['apiName'] == 'WAP_PAY') {
			
			$result = sprintf(
				"apiName=%s&apiVersion=%s&platformID=%s&merchNo=%s&orderNo=%s&tradeDate=%s&tradeAmt=%s&notifyUrl=%s&returnUrl=%s&merchParam=%s",
			$data['apiName'], $data['apiVersion'], $data['platformID'], $data['merchNo'], $data['orderNo'], $data['tradeDate'], $data['tradeAmt'], $data['notifyUrl'], $data['returnUrl'], $data['merchParam']);
